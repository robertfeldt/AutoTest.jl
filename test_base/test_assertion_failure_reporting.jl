...
facts("Reporting failed comparison") do
  ...
  context("t macro") do
    ...
    @fact( @asrt(1==2) => (:fail, "Expected 1 == 2 which is NOT true") )

    @fact( @asrt(1!=1) => (:fail, "Expected 1 != 1 which is NOT true") )

    @fact( @asrt(false) => (:fail, "Expected false which is NOT true") )
  end

  context("report failed comparison") do

    @fact report_failed_comparison("1 == 2", "", "") => "Expected 1 == 2 which is NOT true"
